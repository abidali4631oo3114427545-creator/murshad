...
export function useAnalyticsData() {
  ...
  const { data: realStats, loading: firestoreLoading } = useDoc(statsRef)

  const [data, setData] = useState<AnalyticsData>(INITIAL_EMPTY_DATA)
  const [isUsingRealData, setIsUsingRealData] = useState(false)

  // Initialize with empty state (no simulation) to respect "original only" request
  useEffect(() => {
    if (!isUsingRealData) {
      setData({
        activeUsers: 0,
        ...
        geoHotspots: [
          { id: 'us', name: 'Primary Node', users: 0, coords: [40, -74] },
          { id: 'eu', name: 'Secondary Node', users: 0, coords: [51, 0] },
        ]
      })
    }
  }, [isUsingRealData])
  ...
  useEffect(() => {
    if (realStats && typeof realStats.activeUsers === 'number') {
      ...
      setData(prev => ({
        ...
        activeUsersHistory: Array.isArray(realStats.activeUsersHistory) 
